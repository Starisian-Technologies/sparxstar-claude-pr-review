...
- **Failure handling, and where it actually happens.** Unset composer-resolver configuration fails in `build-context` before any token is minted — that one is genuinely fail-fast. A missing pull request number, an empty diff and a missing `ANTHROPIC_API_KEY` are checked in `review`, which declares `needs: build-context`. So a `push` or `workflow_dispatch` run has already minted both App tokens, checked out the registries and uploaded the trusted-context artifact by the time it stops. No review is produced, but the privileged work is not skipped, and calling that fail-fast overstates it
