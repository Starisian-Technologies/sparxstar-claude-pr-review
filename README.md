...
- Callers must use the `pull_request` trigger. **`push` fails**: the `Get PR diff` step exits 1 when the event payload carries no pull request number, so a push run stops before any review is produced. **`pull_request_target` is forbidden but NOT blocked** — it supplies `github.event.pull_request.number` just as `pull_request` does, so it passes that same check and the review job runs, checking out the PR test-merge with a read-write token in the base-repo context. There is no event-name guard in this workflow; `workflow_call` does not restrict the invoking event, and nothing here rejects `pull_request_target` at runtime. Not using it is the caller's responsibility
